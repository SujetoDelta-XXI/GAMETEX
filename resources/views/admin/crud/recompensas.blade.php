...
@section('crudAdm')
    ...
    <!-- Drawer component -->
                ...
                <div class="space-y-4 sm:space-y-6">
                    <div>
                        <label for="cantidad" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Cantidad</label>
                        <input type="number" name="cantidad" id="cantidad" value="{{ $recompensa->cantidad }}"
                    ...
                    <div>
                        ...
                        <select id="recompensa_tipo_id" name="recompensa_tipo_id"
                            ...
                            @foreach ($recompensasTipos as $tipo)
                                <option value="{{ $tipo->id }}" {{ $recompensa->recompensa_tipo_id == $tipo->id ? 'selected' : '' }}>{{ $tipo->nombre }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
