    ...
    <div class="mt-4">
        ...
        <nav class="bg-gray-900 p-6 rounded-lg">
            <form action="{{ route('admin.crud.evento') }}" method="GET" class="flex flex-col sm:flex-row items-center gap-4">
                <input type="text" name="search" placeholder="Buscar..." class="w-full sm:w-64 px-4 py-2 text-gray-800 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ request('search') }}">
                <select name="search_type" class="w-full sm:w-auto px-4 py-2 text-gray-800 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
